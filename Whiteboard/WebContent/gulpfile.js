...
var handlebarOpts = {
	...
	helpers: {
		...
		assetPathASSETS: function (path, context) {
			return context.data.root[path];
		}
	},
...
gulp.task('assets', function() {
	return gulp.src('./src/assets/**/*')
		...
		.pipe(rev())
		.pipe(gulp.dest('./public'))
		.pipe(rev.manifest('rev-manifest-assets.json'))
		.pipe(gulp.dest('./'));
});

//unversioned assets
gulp.task('unsafe_assets', function() {
	return gulp.src('./src/unsafe_assets/**/*')
		.pipe(plumber(plumberErrorHandler))
		.pipe(gulp.dest('./public'));
});
...
gulp.task('html', function() {
	...
	try {
		var manifest = JSON.parse(fs.readFileSync('./rev-manifest-assets.json', 'utf8'));
		for (i in manifest) templateData[i] = manifest[i];
	} catch(e) {};
...
gulp.task('watch', function() {
	...
	//watch unsafe asset files
	gulp.watch('./src/unsafe_assets/**/*.*', ['default'])

	//watch .hbs files
	gulp.watch('./src/templates/**/*.hbs', ['default']);

	//watch robot.txt file
	gulp.watch('./src/robots.txt', ['default']);
});

//default
gulp.task('default', function(callback) {
	runSequence(['styles', 'scripts', 'images', 'favicons', 'assets', 'unsafe_assets', 'repos', 'robots'], 'fonts', 'html', callback);
});